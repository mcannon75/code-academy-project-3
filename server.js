...
// Function to create a new comment in the datatbase
function createComment(url, request) {
  const requestComment = request.body;
  const response = {};

  function validRequest() {
    if (request.hasOwnProperty('body') && request.body.hasOwnProperty('comment')) {
      return true;
    } else {
      return false;
    }
  ...
  function requestValuesSupplied() {
    if (database.users[requestComment.comment.username] && database.articles[requestComment.comment.articleId] && requestComment.comment.username !== '' && requestComment.comment.articleId !== '' && requestComment.comment.body !== '') {
      return true;
    } else {
      return false;
    }
  ...
  if (validRequest() && requestValuesSupplied()) {
    const comment = {
...
// Funciton to update an existing comment in the database
function updateComment(url, request) {
  ...
  function validComment() {
    if (Object.keys(request).length === 0 || request.body.hasOwnProperty('comment') === false) {
      return false;
  ...
  // Check the username in the comment is valid
  function validUsername() {
    if (Object.keys(request).length === 0 || database.comments[id] === undefined) {
      return false;
    } else if (request.body.comment.username === database.comments[id].username){
      return true;
    } else {
      return false;
    }
  ...
  // Check the comment ID provided is valid
  function validCommentID() {
    if (Object.keys(request).length === 0) {
      return false;
    } else if (id === request.body.comment.id) {
      return true;
    } else {
      return false;
    }
  ...
  if (validComment() === false) {
    response.status = 400;
  } else if (validComment() && validUsername() && validCommentID()) {
    const requestComment = request.body;
    const comment = {
      id: id,
      body: requestComment.comment.body,
...
// Function to delete an existing comment from the database
function deleteComment(url) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const response = {};
  let articleId;
  let userId;
  let articleCommentArrayPosition;
  let userCommentArrayPosition;

  // Check for a valid comment ID, set variables if valid
  function validCommentId() {
    if (database.comments[id] === undefined) {
      return false;
  } else {
    articleId = database.comments[id].articleId;
    userId = database.comments[id].username;
    articleCommentArrayPosition = database.articles[articleId].commentIds.indexOf(id);
    userCommentArrayPosition = database.users[userId].commentIds.indexOf(id);
    return true;
  }
}
  ...
  if (validCommentId()) {
    ...
    database.articles[articleId].commentIds.splice(articleCommentArrayPosition, 1);
    database.users[userId].commentIds.splice(userCommentArrayPosition, 1);
...
// Function to upvote an existing comment
function upvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const response = {};

  // Ensure user is a valid user in our database
  function validUser() {
    if (database.users[request.body.username] === undefined ){
      return false;
    } else {
      return true;
    }
  ...
  // Ensure comment ID is valid and user has not already upvoted this comment
  function validCommentAndCanUpvote() {
    if (database.comments[id] === undefined) {
      return false;
    } else {
      const userIndex = database.comments[id].upvotedBy.findIndex(element => element = request.body.username);
      if (request.body.username === database.comments[id].upvotedBy[userIndex]) {
        return false;
      } else {
        return true;
      }
  ...
  // Check is user has previously downvoted this comment and if so, remove that record from the database
  function checkForPreviousDownvote() {
    if (database.comments[id] === undefined) {
      return false;
    } else {
      const userIndex = database.comments[id].downvotedBy.findIndex(element => element = request.body.username);
      if (userIndex >= 0 && request.body.username === database.comments[id].downvotedBy[userIndex]) {
        database.comments[id].downvotedBy.splice(userIndex, 1)
      } else {
        return false;
      }
    }
  ...
  if (request.hasOwnProperty('body') === false) {
    response.status = 400;
  } else if (validUser() && validCommentAndCanUpvote()) {
    database.comments[id].upvotedBy.push(request.body.username);
    checkForPreviousDownvote();
    response.status = 200;
    response.body = {};
...
// Function to downvote an existing comment
function downvoteComment(url, request) {
  ...
  // Ensure comment ID is valid and user has not already downvoted this comment
  function validCommentAndCanDownvote() {
    if (database.comments[id] === undefined) {
      return false;
    } else {
      const userIndex = database.comments[id].downvotedBy.findIndex(element => element = request.body.username);
      if (request.body.username === database.comments[id].downvotedBy[userIndex]) {
        return false;
      } else {
        return true;
      }
  ...
  // Check is user has previously upvoted this comment and if so, remove that record from the database
  function checkForPreviousUpvote() {
    if (database.comments[id] === undefined) {
      return false;
    } else {
      const userIndex = database.comments[id].upvotedBy.findIndex(element => element = request.body.username);
      if (userIndex >= 0 && request.body.username === database.comments[id].upvotedBy[userIndex]) {
        database.comments[id].upvotedBy.splice(userIndex, 1)
      } else {
        return false;
      }
    }
  ...
  if (request.hasOwnProperty('body') === false) {
    response.status = 400;
  } else if (validUser() && validCommentAndCanDownvote()) {
    database.comments[id].downvotedBy.push(request.body.username);
    checkForPreviousUpvote();
    response.status = 200;
    response.body = {};
    response.body.comment = database.comments[id];
  } else {
